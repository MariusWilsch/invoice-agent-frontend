...
const Login = () => {
  ...
  const signInWithPasswordAndCheckAAL = async (email, password) => {
    ...
    const { data: aal, error: aalError } = await getAuthenticatorAssuranceLevel();
  ...
  const isFullyAuthenticated = (aal) => aal.currentLevel === "aal2" && aal.nextLevel === "aal2";
  const needsMFAVerification = (aal) => aal.currentLevel === "aal1" && aal.nextLevel === "aal2";
  const needsMFAEnrollment = (aal) => aal.currentLevel === "aal1" && aal.nextLevel === "aal1";
  ...
  const handleMFAEnrolled = async () => {
    try {
      ...
      toast.error(`MFA enrollment failed`, {
        description: error.message || "An error occurred during MFA enrollment.",
      });
      await logout();
    }
  };
